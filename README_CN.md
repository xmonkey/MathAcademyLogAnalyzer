...
## 从 Math Academy 获取活动日志

### 如何下载 PDF 活动日志

使用本工具前，您需要先从 Math Academy 下载活动日志 PDF：

1. **登录家长/监督账户**
   - 访问 [mathacademy.com](https://mathacademy.com)
   - 使用您的家长或监督账户凭据登录

2. **进入学生设置**
   - 点击学生姓名旁边的设置图标（⚙️）
   - 从菜单中选择 **"Documentation"**（文档）

3. **请求活动日志**
   - 在 **"Activity Log"**（活动日志）部分，点击 **"Request..."**（请求...）
   - 设置您想要的 **Time Frame**（时间范围）（例如：最近30天、最近3个月、自定义范围）
   - 点击 **"Preview"**（预览）生成活动日志

4. **下载 PDF**
   - 等待预览加载完成
   - 使用浏览器的下载选项下载 PDF 文件
   - 保存为描述性的文件名（例如：`student_activity_log_2025-01.pdf`）

### 最佳实践建议
- **时间范围**：选择合理的时间范围（1-6个月）以获得最佳处理效果
- **文件命名**：使用描述性名称以便轻松识别不同的报告
- **定期下载**：每月下载报告以保持持续跟踪
- **浏览器选择**：使用 Chrome 或 Firefox 以获得最佳兼容性
